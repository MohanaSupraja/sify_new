...
app.put('/update-category', authenticateToken, authorizeRoles('Developer'), async (req, res) => {
  const { label, name, category } = req.body;
  try {
    console.log(label, name, category)
    if (label == 'Well Worn') {
      const snapshot = await firestore.collection('products').where('source_id', '==', 2).get();
      const query = snapshot.where('Name', '==', name).get();
      await query.update({ Category: category });
    }
    else {
...
app.post('/add-activity', authenticateToken, authorizeRoles('Developer'), async (req, res) => {
  const { websiteName, websiteElementId } = req.body;
  ...
  try {
    ...
    // Query the collection to check if the websiteName already exists
    const snapshot = await firestore.collection('masterActivities')
      .where('activityName', '==', websiteName)
      .get();

    let activityId;
    const length = await firestore.collection('masterActivities').get();
    const id = length.size + 1;

    if (!snapshot.empty) {
      const existingActivity = snapshot.docs[0].data();
      activityId = existingActivity.activityId + 1;
    } else {
      activityId = 1;
    }

    // Directly set the values in Firestore without creating a new object
    await firestore.collection('masterActivities').doc().set({
      activityName: websiteName,
      status: 'Inactive',
      date: date,
      websiteName: websiteName,
      activityId: activityId,  // Incrementing or initializing activityId
      websiteElementId: websiteElementId,
      id: id
    });

    // Send success response
    res.status(201).json({ message: `${websiteName} activity added successfully` });
  } catch (error) {
...
app.get('/get-activities', authenticateToken, authorizeRoles('Developer'), async (req, res) => {
  try {
    ...
    const snapshot = await firestore.collection('masterActivities').get();
...
app.post('/remove-activities', authenticateToken, authorizeRoles('Developer'), async (req, res) => {
  try {
    const { websiteName, activityName, activityId } = req.body;

    console.log("websiteName:", websiteName, activityName);
    const activitiesRef = firestore.collection('masterActivities');
    const querySnapshot = await activitiesRef
      .where('activityId', '==', activityId)
      .get();

    if (querySnapshot.empty) {
      console.log(`No activity '${activityId}' found for website: ${websiteName}`);
      return res.status(404).json({ message: `ActivityId '${activityId}' not found for website: ${websiteName}` });
    }

    // Loop through the results and delete the matching document(s)
    querySnapshot.forEach(async (doc) => {
      await doc.ref.delete();
    });

    console.log(`Activity '${activityName}' successfully removed for website: ${websiteName}`);
    return res.status(200).json({ message: `Activity '${activityName}' successfully removed for website: ${websiteName}` });
  } catch (error) {
...
app.post('/update-activities', authenticateToken, authorizeRoles('Developer'), async (req, res) => {
  try {
    ...
    if (querySnapshot.empty) {
      return res.status(404).json({ message: `Activity '${activityName}' not found for website: ${websiteName}` });
    }
    if (status === "Active") {
      ...
      activitiesSnapshot.forEach(doc => {
        ...
        if (activity.status === "Active" && !(doc.id === id)) {
          isActiveFound = true;
        }
      });
    ...
    // Update the first matching document (if multiple matches, only the first one is updated)
    const docToUpdate = querySnapshot.docs[0];
    const docRef = docToUpdate.ref;

    // Update the activity in the document
    await docRef.update({
      id,
      activityName,
      status,
...
app.post('/add-sylviecategory', authenticateToken, authorizeRoles('Developer'), async (req, res) => {
  try {
    ...
    await categoriesRef.doc().set({ name: category, id });
